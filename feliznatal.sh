...
#+ O nome tb pode ser passado como parÃ¢metro.

NomeIni=(${*:-Alyce Suza})

#  Feliz Ano ... 
#+ Se jÃ¡ passou do meio do ano, este ano mais 1, senÃ£o este ano
Ano=$[$(date +%_j)>365/2?$(date +%Y)+1:$(date +%Y)]

function MontaArr
{
#  Monta um array Arr[$Lin$Col] (ambas com zeros Ã  esqueda) 
#+ com o caracteres que estÃ¡ naquela posiÃ§Ã£o
    local i
    local l
    for ((i=0; i<${#3}; i++))
    {
...
# UltLin="SÃ£o os votos de "$Nome" para vocÃª e sua famÃ­lia"
...
k=1
                   ...
                   # Pendurando as bolas (da Ã¡rvore, claro!)
while true
do
    for ((i=1; i<=35; i++))
    {
                   # Apagando a bola que foi ligada hÃ¡ 35 rodadas atras
        [ $k -gt 1 ] && {
